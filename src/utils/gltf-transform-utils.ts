import { JSONDocument, WebIO, Document } from "@gltf-transform/core";
 ...
 * @param  {} doc
 */
export async function gtWriteToSeparateGltf(doc: Document) {
  const options = {
    basename: "model",
  };
  const obj = await io.writeJSON(doc, options);
  // {
  //   json:{...},// 一个对象，对应
